...
function buildFamilyMap(persons, relationships) {
  ...
  relationships.forEach(rel => {
    if (rel.relationship_type === 'parent') {
      if (map[rel.person1_id]) map[rel.person1_id].childIds.push(rel.person2_id);
    ...
    if (rel.relationship_type === 'spouse') {
      if (map[rel.person1_id] && !map[rel.person1_id].spouseIds.includes(rel.person2_id))
        map[rel.person1_id].spouseIds.push(rel.person2_id);
      if (map[rel.person2_id] && !map[rel.person2_id].spouseIds.includes(rel.person1_id))
...
function calculatePositions(persons, map, generations) {
  ...
  for (let gen = 0; gen <= maxGen; gen++) {
    ...
    roots.forEach(id => {
      ...
      const childrenWidth = allChildren.length > 0
        ? allChildren.length * (NODE_W + H_GAP) - H_GAP
        : 0;

      const coupleWidth = person.spouseIds.length > 0
        ? NODE_W * 2 + COUPLE_GAP
        : NODE_W;
    ...
    ids.forEach(id => {
      ...
      if (person.parentIds.length > 0) {
        ...
        if (parentPositions.length > 0) {
          const parentAvgX = parentPositions.reduce((a, b) => a + b, 0) / parentPositions.length;
          // Get siblings
...
function FamilyTreeSVG({ persons, relationships, onPersonClick }) {
  ...
  persons.forEach(p => {
    ...
    if (allChildren.length > 0) {
      ...
      lines.push(
        <line key={`pv-${p.id}`} x1={parentMidX} y1={parentBottomY}
          x2={parentMidX} y2={midY} stroke="#95a5a6" strokeWidth={2} />
      );

      if (allChildren.length > 1) {
        const childXs = allChildren.map(cId => positions[cId]?.x + NODE_W / 2).filter(Boolean);
        const hLeft = Math.min(...childXs);
        const hRight = Math.max(...childXs);
        lines.push(
          <line key={`ph-${p.id}`} x1={hLeft} y1={midY}
            x2={hRight} y2={midY} stroke="#95a5a6" strokeWidth={2} />
        );
      }
      ...
      allChildren.forEach(cId => {
        ...
        lines.push(
          <line key={`cv-${p.id}-${cId}`} x1={childTopX} y1={midY}
            x2={childTopX} y2={childPos.y} stroke="#95a5a6" strokeWidth={2} />
        );
      });
    }
  });
  ...
  return (
    ...
    >
      ...
      {persons.map(p => {
        ...
        return (
          <g key={p.id} onClick={() => onPersonClick(p.id)}
...
export default function TreePage() {
  ...
  const handleAddPerson = async () => {
    ...
    try {
      await axios.post('/persons', form, { headers: { 'Content-Type': 'application/json' } });
      toast.success('Person added');
      setShowModal(false);
      setForm({ first_name: '', last_name: '', gender: 'male', birth_date: '', death_date: '', bio: '' });
      fetchData();
    } catch (err) {
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        ...
        <button onClick={() => setShowModal(true)} style={styles.btn}>+ Add Person</button>
      </div>

      <input style={styles.search} placeholder="🔍 Search family members..."
        ...
        <>
          ...
          <div style={styles.grid}>
            {filteredPersons.map(p => (
              <div key={p.id} style={styles.card} onClick={() => navigate(`/person/${p.id}`)}>
                <div style={{ ...styles.avatar, backgroundColor: p.gender === 'female' ? '#8e44ad' : '#2c3e50' }}>
                  <span style={styles.initials}>
                    {p.first_name[0]}{p.last_name ? p.last_name[0] : ''}
                  </span>
                </div>
                <p style={styles.name}>{p.first_name} {p.last_name}</p>
                <p style={styles.meta}>{p.gender || 'Unknown'}{p.birth_date ? ` · ${new Date(p.birth_date).getFullYear()}` : ''}</p>
              </div>
            ))}
          </div>
          ...
          <div style={styles.treeSection}>
            ...
            <p style={styles.treeHint}>Scroll to zoom · Drag to pan · Click a person to view profile · <span style={{ color: '#e74c3c' }}>── spouse</span></p>
            <div
            ...
            <div style={styles.zoomControls}>
              ...
              <button onClick={() => setZoom(1)} style={styles.zoomBtn}>Reset</button>
              <button onClick={() => setZoom(z => Math.max(0.3, z - 0.1))} style={styles.zoomBtn}>−</button>
            </div>
          </div>
        </>
      )}
        ...
        <div style={styles.overlay}>
          <div style={styles.modal}>
            ...
            <div style={styles.modalButtons}>
              <button onClick={handleAddPerson} style={styles.btn}>Save</button>
              <button onClick={() => setShowModal(false)} style={styles.btnCancel}>Cancel</button>
            </div>
          </div>
        </div>
      )}
...
const styles = {
  ...
  btn: { backgroundColor: '#2c3e50', color: 'white', border: 'none', padding: '0.6rem 1.4rem', borderRadius: '6px', cursor: 'pointer', fontSize: '1rem' },
  btnCancel: { backgroundColor: '#95a5a6', color: 'white', border: 'none', padding: '0.6rem 1.4rem', borderRadius: '6px', cursor: 'pointer', fontSize: '1rem' },
  search: { width: '100%', padding: '0.7rem 1rem', borderRadius: '8px', border: '1px solid #ccc', fontSize: '1rem', marginBottom: '1.5rem', boxSizing: 'border-box', backgroundColor: 'white' },
  center: { textAlign: 'center', marginTop: '3rem', color: '#888' },
  empty: { textAlign: 'center', marginTop: '5rem', color: '#555', fontSize: '1.1rem' },
  grid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '1rem', marginBottom: '2rem' },
  card: { backgroundColor: 'white', borderRadius: '10px', padding: '1.2rem', textAlign: 'center', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' },
  avatar: { width: '64px', height: '64px', borderRadius: '50%', margin: '0 auto 0.6rem', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  initials: { color: 'white', fontSize: '1.3rem', fontWeight: 'bold' },
  name: { fontWeight: '600', color: '#2c3e50', margin: '0.2rem 0', fontSize: '0.95rem' },
  meta: { color: '#888', fontSize: '0.78rem', margin: 0, textTransform: 'capitalize' },
  ...
  treeBox: { backgroundColor: 'white', borderRadius: '12px', height: '600px', boxShadow: '0 2px 12px rgba(0,0,0,0.08)', overflow: 'hidden', cursor: 'grab', position: 'relative' },
  zoomControls: { display: 'flex', gap: '0.5rem', marginTop: '0.5rem', justifyContent: 'flex-end' },
  zoomBtn: { backgroundColor: '#2c3e50', color: 'white', border: 'none', padding: '0.4rem 0.8rem', borderRadius: '4px', cursor: 'pointer', fontSize: '1rem' },
  overlay: { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 },
  modal: { backgroundColor: 'white', padding: '2rem', borderRadius: '10px', width: '100%', maxWidth: '440px', maxHeight: '90vh', overflowY: 'auto' },
  modalTitle: { color: '#2c3e50', marginBottom: '1.2rem' },
  modalButtons: { display: 'flex', gap: '1rem', marginTop: '1.2rem' },
  field: { marginBottom: '0.9rem' },
  label: { display: 'block', marginBottom: '0.3rem', color: '#444', fontSize: '0.88rem', fontWeight: '500' },
  input: { width: '100%', padding: '0.6rem', borderRadius: '6px', border: '1px solid #ddd', fontSize: '1rem', boxSizing: 'border-box' }
};